...
var self = module.exports = {
  ...
  get_tag_links: function(tag_id) {
    ...
    request.get(
      settings.get('API') + '/tags/' + tag_id + '/links',
      {
        json: true
      },
      function(error, response, body) {
  ...
  find_by_url: function(url) {
    var deferred = q.defer();
    request.get(
      settings.get('API') + '/links/findOne?filter[where][url]=' + url,
      {
        json: true
      },
      function(error, response, body) {
  ...
  add_link: function(url, description, id=null) {
    var deferred = q.defer();
    var jsonData = {};
    jsonData['url'] = url;
    ...
    request.post(
      settings.get('API') + '/links', { json: jsonData },
      function(error, response, body) {
        if (!error && response.statusCode == 200) {
          return deferred.resolve(body);
        } else {

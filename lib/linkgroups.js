...
var self = module.exports = {
  ...
  get_linkgroups: function() {
    var deferred = q.defer();
    request.get(
      settings.get('API') + '/linkgroups', { json: true },
      function(error, response, body) {
        if (!error && response.statusCode == 200) {
          return deferred.resolve(body);
        } else {
  ...
  add_linkgroup: function(link_id, group_id, id=null) {
    var deferred = q.defer();
    var jsonData = {};
    jsonData['linkId'] = link_id;
    jsonData['groupId'] = group_id;
    ...
    request.post(
      settings.get('API') + '/linkgroups', { json: jsonData },
      function(error, response, body) {
        if (!error && response.statusCode == 200) {
          return deferred.resolve(body);
        } else {
